...
/* Static variables */
static const int start_grid_x = 17, start_grid_y = 27;		// where to put pacman at the beginning
...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
	...
	int col= pacman->objData.Coord.x, row= pacman->objData.Coord.y;
	
	switch (targetDirec)
	{
	case UP:
		if (!is_wall_block(M, col, row - 1) && !is_room_block(M, col, row - 1))
			return true;
		else
			return false;
	case DOWN:
		if (!is_wall_block(M, col, row+1) && !is_room_block(M, col, row+1))
			...
			return false;
	case LEFT:
		if (!is_wall_block(M, col-1, row) && !is_room_block(M, col-1, row))
			return true;
		else
			return false;
	case RIGHT:
	...
	if (is_wall_block(M, col, row) || is_room_block(M, pacman->objData.Coord.x, pacman->objData.Coord.y))
		return false;
...
Pacman* pacman_create() {
	...
	*/
	...
	/* TODO? */
	/* hint / just put it */
	pman->objData.Coord.x = 17;
	pman->objData.Coord.y = 26;
	...
	else if (charactor_two) {
		pman->move_sprite = load_bitmap("Assets/pacman_blue_move.png");
		pman->die_sprite = load_bitmap("Assets/pacman_blue_die.png");
	}
	else if (charactor_three) {
		pman->move_sprite = load_bitmap("Assets/pacman_pink_move.png");
		pman->die_sprite = load_bitmap("Assets/pacman_pink_die.png");
	}
	return pman;
...
void pacman_draw(Pacman* pman) {
	...
	*/
	...
	if (game_over) {
		...
		*/
		offset = al_get_timer_count(pman->death_anim_counter) / 10;
		if (offset == 12) {
...
void pacman_move(Pacman* pacman, Map* M) {
	...
	{
	case UP:
		if (pacman->objData.Coord.y == 1 && pacman->objData.Coord.x == 34) {
			pacman->objData.Coord.y = 29;
			pacman->objData.Coord.x = 1;
		}
		else pacman->objData.Coord.y -= 1;
		pacman->objData.preMove = UP;
		break;
	case DOWN:
		if (pacman->objData.Coord.y ==28 && pacman->objData.Coord.x == 1) {
			pacman->objData.Coord.y = 0;
			pacman->objData.Coord.x = 34;
		}
		else pacman->objData.Coord.y += 1;
...
void pacman_toggle_SPEED(Pacman* pman, bool setFLEE) {
	// [TODO]
	...
	if (setFLEE) {
		if (pman->speed == basic_speed) {
			pman->speed = basic_speed * 2;
		}
	}
	else {
		if (pman->speed == basic_speed * 2)
			pman->speed = basic_speed;
	}
}
