...
static void ghost_pink_move_script_FREEDOM(Ghost* ghost, Map* M, const Pacman* const pacman) {
	// [HACKATHON 2-4]
	...
	for (Directions i = 1; i <= 4; i++)
		if (ghost_movable(ghost, M, i, 1) && ghost->objData.facing + i != 5&&i != 5-shortestDirection)	proba[cnt++] = i;
	if(cnt)ghost_NextMove(ghost, proba[generateRandomNumber(0, cnt-1)]);
	else ghost_NextMove(ghost, 5 - ghost->objData.facing);
